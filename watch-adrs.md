...
## ADR-044 — Replace bootstrap-admin with a fenced provisioner role, and publish the policy
**Status:** Accepted · *Supersedes the "admin for now" stance in ADR-020 and the `gha-apply` AdministratorAccess TODO. Realizes the "Not yet done → scope down the apply path" item in platform `docs/architecture/accounts.md`.*

**Context.** Everything in this project is deployable except the thing that makes it deployable *by someone else*. `watch-bootstrap` — an unrotated admin credential — provisions the estate, deploys to it, and verifies it; `gha-apply` holds `AdministratorAccess` with the comment "the OIDC trust is the real boundary"; `member-access` defaults to `AdministratorAccess`. Any organisation adopting this repo must hand its security team a policy describing what the deployment actually needs, and today the honest answer is "administrator, trust us." That sentence is the single largest thing standing between this codebase and a real engagement — bigger than any feature, because no feature gets deployed without it. Dave (2026-07-13): *"they're gonna have to give to their security team a policy that has specifics of what is needed to run this project."*

**Decision.** Publish the permissions as a **reviewable artifact**, and make the code prove it.
1. **Three identities, not one.** `watch-provisioner` (OpenTofu's identity: create/destroy the estate), `watch-ci-plan` (read-only, already existed), `watch-ro` (humans verifying). The admin credential is needed for exactly **one** step — minting the provisioner — and then never again.
2. **The policy documents ARE the deliverable.** Five plain IAM JSON files in `platform/policies/`, placeholdered (`${project}`/`${region}`/`${account_id}`) so no account id sits in a public repo and one set of documents serves every adopter; `make policies` renders them concrete. A reviewer reads IAM JSON, not Terraform. Terraform merely applies them.
3. **The IAM grant is fenced by a permissions boundary — this is the crux.** The provisioner must create ~25 roles, so it needs `iam:CreateRole` + `iam:AttachRolePolicy`, which **unfenced is a two-call path to administrator**. So: `iam:CreateRole` is conditioned on `iam:PermissionsBoundary` = `watch-boundary`, the boundary **denies all IAM writes** to every role it caps, and the provisioner is explicitly denied any power over the boundary, its own policies, or its own role. It also cannot mint an IAM user, an access key, or a login profile — no long-lived credential can come out of this. Every one of the estate's 25 roles carries the boundary (plumbed through the Terragrunt root, so ~20 stack files stayed untouched).
4. **CI apply holds no powers at all.** `gha-apply`'s entire policy becomes `sts:AssumeRole` on the provisioner (in the hub and in each member); the root config learns to assume a role **in-account** (`WATCH_ASSUME_IN_ACCOUNT=1`), which it previously only did across accounts. Whatever CI can do is now, by construction, exactly what the security team read.
5. **The fence is gated, not merely intended.** `make policy-check` (pre-commit + PR) fails if a document is invalid or oversized, if any `Allow *:*` appears outside the boundary's deliberate ceiling statement, if `iam:CreateRole` loses its boundary condition, or if **any** `aws_iam_role` in the repo stops carrying the boundary. A fence that can rot silently is not a fence.
6. **Proven by use, not by reading.** A full `make live` → `make live-verify` → `make teardown` as the provisioner **with no admin credential in the environment**, once per topology. Teardown is the part that matters: a missing `Delete*` does not surface until you try to leave. Re-proving this is now item 6 of the **major-release checklist**, so the published policy cannot drift from what an adopter signed off on without the release failing.
7. **Honesty about what could not be scoped.** `docs/SECURITY.md` carries an explicit section listing every place we left `Resource: "*"` and why (EC2 create actions have no pre-existing ARN and several of the resources cannot carry tags; ECS/Lambda/SFN/CodeBuild resource-level support is uneven), plus the compensating controls (region conditions, and explicit denies on `RunInstances`, peering, VPN, snapshot-sharing, and key/secret replication — the escape and exfiltration verbs). A reviewer looks for this section first; its absence is what makes a policy look like marketing.

**Consequences.**
- *Gain:* the project becomes adoptable. An adopter hands their security team five JSON files and a document that says what each is for, what it cannot do, and how it was proven — instead of asking for administrator.
- *Gain:* the blast radius of a compromised CI is now bounded by a document, not by a trust policy alone. `gha-apply` cannot create an IAM user, cannot escalate, cannot leave the region, cannot touch the org.
- *Cost:* a real bootstrap step (one admin apply) that we must document rather than hand-wave; and any new AWS service a future stack uses now needs its actions added to a policy **and** re-proved live — which is friction by design, since that friction is the thing an adopter is buying.
- *Cost / caveat:* the boundary means a compromised **provisioner** could still do damage inside these accounts (it is, after all, the thing that builds them). The controls that bound it are: it cannot create durable credentials, cannot escape the region, cannot touch the organisation or billing, cannot delete the audit trail, and cannot weaken its own fence — so an attacker holding it has a loud, time-boxed, account-scoped session, not a foothold.
- *Guardrail:* `make policy-check` in pre-commit + CI; routing of the new stacks asserted by the existing Terratest topology matrix; the live rehearsal per topology tracked as platform#55.

**As built.** `platform/policies/{watch-boundary,watch-provisioner-core,watch-provisioner-data,watch-provisioner-delivery,watch-provisioner-iam}.json`, `modules/provisioner-role`, stacks `account/provisioner` + `member-iam/{nonprod,prod}`, `permissions_boundary` on all 25 estate roles, root `WATCH_BOUNDARY` / `WATCH_ASSUME_IN_ACCOUNT` knobs, `gha-apply` reduced to assume-only, `scripts/policy-check.sh` + `scripts/render-policies.sh` (`make policy-check` / `make policies`), `docs/SECURITY.md` (the handout), major-release checklist item 6.

## ADR-045 — An account-global name has exactly one owner, and the topology decides who
**Status:** Accepted · *Refines ADR-020 (multi-account split) and ADR-044 (the fenced provisioner). Found by the ADR-044 live rehearsal.*

**Context.** The three-topology rehearsal (single-account; two members of a new org; two members of an existing org) was supposed to prove the *policies*. It proved them — zero denials in every topology, the documents transfer unchanged. What it found instead was a **design** mistake, and Dave named it before the code did: *"why would there be an attempt to make two of these GitHub OIDC connections… this is not actually a bug per se but it's revealing a mistake in the design."*

Some AWS names are **account-global**: an IAM OIDC provider (one per URL per account), and — because we deliberately name them per-*project* so that one `WATCH_MEMBER_ROLE_NAME` works everywhere — `watch-provisioner` and `watch-boundary`. Three stacks created a provisioner; two created an OIDC provider. Nobody *owned* either. Ownership was **positional**: it worked only because Terragrunt routing happened to send each creator to a different account. Collapse the topology to one account and the creators land on top of each other (`409 EntityAlreadyExists`) — and, far worse for an adopter, a member account that **already federates GitHub Actions** (which any real org does) would have collided with *their* provider on day one. Self-provisioning an account-global singleton is never correct in an account you do not own.

A second instance of the same confusion: a blank member id meant both *"this stack lives in the current account"* and *"there is no separate member account"*. The root coalesced the first meaning; `prod/deploy` read the raw value and rendered `arn:aws:iam:::root`, which IAM rejects. One rule, re-derived ad hoc in four places, wrong in one.

**Decision.**
1. **Name the two meanings, in one place.** `accounts.hcl` exposes `*_account_id` (*where a stack lives* — always resolves; blank falls back to the current account) and `has_*` (*whether a separate member account exists* — the **only** cross-account signal). Nothing reads `WATCH_*_ACCOUNT_ID` directly any more.
2. **Every account-global name gets an explicit owner stack, and consumers take an ARN.** `modules/oidc-provider` owns the GitHub federation entry; `account/oidc-provider` (hub) and `member-oidc/nonprod` (the pipeline's account) are its only owners. `modules/ci-pipeline-trigger` and `modules/github-oidc` now **consume** the ARN and create nothing — the ARN is fully determined by account + URL, so this costs no dependency.
3. **A non-owner creates nothing.** Owner stacks gate on a `create` input driven by the topology (`has_nonprod` / `has_prod`). In single-account, `member-iam/*` and `member-oidc/*` are deliberate no-ops because the hub's stacks already own those names — not second claimants racing for them.
4. **Adopters never lose a fight they didn't enter.** `WATCH_GITHUB_OIDC_EXISTS=1` adopts an account's existing GitHub federation instead of creating one. We do not destroy or contest infrastructure we did not create.
5. **Two federation points, and only two — by design, not by accident.** GitHub federates into the **hub** (the platform repo's `gha-plan`/`gha-apply`) and into the **pipeline's account** (the watch repo's `watch-ci-trigger`, whose entire power is `StartPipelineExecution`). **Prod federates GitHub nowhere**: nothing in GitHub reaches prod directly — the nonprod pipeline does, by assuming `watch-prod-deploy`. Collapsing the second entry into the hub would be *worse*, not tidier: it would hand the **application** repo a trust relationship with the **management** account.

**Consequences.**
- *Gain:* the repo can be applied into an existing org's accounts without colliding with what is already there — the single most likely first-contact failure for an adopter, and one no amount of policy review would have caught.
- *Gain:* the rule is now checkable without AWS. `TestOneOwnerPerAccountGlobalName` renders all three topologies and fails if two stacks would create the same account-global name in one account (and asserts the estate stays *complete*: every account has a provisioner, the pipeline's account has a federation entry, prod has none). `TestOnlyTheOwnerModuleDeclaresTheOIDCProvider` fails if any other module declares one. Both are mutation-tested — reintroducing the bug makes them fail.
- *Cost:* one more stack per federating account, and a `create` flag that reads like ceremony in the two-member topologies where it is always true. That is the price of ownership being explicit instead of positional.
