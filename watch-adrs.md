...
**Decision.**
1. **Incident stays rich and unchanged** — the *only* type with escalation / SLA / tiers / dedupe / paging. **Problem** and **RCA** are **thin** models: id, human `number`, title, `status` (own lifecycle — Problem: open→investigating→known-error→resolved/closed; RCA: draft→in-review→published), assignee, timestamps, small JSON for the variable bits. No engine leaks out of Incident.
...
3. **One generic link table** (`RecordLink`): GFK `from` + GFK `to` + `kind` (parent-of / groups / references / caused-by / relates), unique(from, to, kind), with `created_by`/`created_at`. Both ends generic, so a single table expresses parent-incident, "this problem groups these 10 incidents," and "this RCA references these 3 incidents + 2 checks" — and it can point at a **Check** (or anything) without touching that model. (Effectively Jira issue-links.)
4. **RCA is a first-class stored record.** Its `document` (markdown) is **seeded by assembly** from its linked records (reuse `rca_markdown` over the union of linked incidents' timelines) and then **hand-edited**; **AI-drafting is a deferred layer** (Watch has no live LLM wired yet).
...
6. **Check stays a probe, untouched** — a subject+verdict, not a ticket; it participates only by being linked. A validated check that warrants an incident goes through the existing `create_incident_idempotent` **service call** (loose coupling), not a hard FK.
